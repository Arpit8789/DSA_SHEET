...
export default function Home({
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-[#070c28] to-[#1d2332] text-slate-100 flex flex-col">
      <Header />
      {/* Make below flex-row for desktop, switch to column on mobile */}
      <div className="w-full flex flex-row justify-center gap-12 mt-12 px-4 flex-col md:flex-row items-stretch">
        {/* Sheet card (left/main) */}
        <div className="w-full max-w-xl bg-[#111329] px-8 py-8 rounded-2xl shadow-2xl border border-[#232543] flex flex-col gap-6">
          ...
          <div className="flex flex-row gap-4 justify-between items-center w-full mb-3">
            {sheets.map(sheet => {
              // Calculate 'done' and 'total' per sheet
        ...
        {/* Motivational quotes panel (right sidebar) */}
